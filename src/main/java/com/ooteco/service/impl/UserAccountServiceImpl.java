...
public class UserAccountServiceImpl implements UserAccountService{
    ...
    public JsonResult regist(String mobile, String password, String smsCode, String ip) {
        ...
        int invitecode = 0;
        do{
            invitecode = (int)((Math.random()*9+1)*10000000);
        ...
        account.setNickname("用户"+System.currentTimeMillis());  //昵称
    ...
    public JsonResult login(String mobile, String password, String ip) {
        ...
        if(account==null) {
            log.error(String.format("user account,login,user is null,mobile:%s,ip:%s", mobile, ip));
            return ResultFactory.newFail("账号或密码错误");
        }
        //登录日志
    ...
    @Override
    public JsonResult editPassword(int userId, String password, String newPassword, String ip) {
        ...
        if(account==null) {
            ...
            return ResultFactory.newFail("旧密码错误");
        }
        //校验新密码是否与旧密码相同
