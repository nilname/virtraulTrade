...
public class UploaderController extends AbstractController{
    ...
    public String image(@RequestParam("file") MultipartFile file, HttpServletRequest request){
        ...
        String filePath = (basePath!=null ? basePath : request.getSession().getServletContext().getRealPath("upload/")) +"image/";
        try {
            ...
            return renderResult(ResultFactory.newSucc(filePath+fileName));
        } catch (Exception e) {
            //e.printStackTrace();
