...
public class AppNewsController extends AbstractController{
    ...
    public String find(@RequestParam("id") int id){
        return renderResult(appNewsService.find(id));
    }
    ...
    public String add(@RequestParam("type") Byte type, @RequestParam("title") String title, @RequestParam("content") String content){
        AppNews appNews = new AppNews();
        appNews.setType(type);
        appNews.setTitle(title);
        appNews.setContent(content);
        appNews.setInputtime(new Date());
        return renderResult(appNewsService.add(appNews));
    }
